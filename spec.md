...
| `event_dates` | string[] | | タイトル・概要から抽出したイベント開催日（`YYYY-MM-DD` の配列）。`scrapers/dateextract.py` が `2026年6月27日` / `6月27日` / `6月27日〜29日` 等の表記から抽出。フロントのカレンダー表示に利用 |
...
- フィルタ: サイト別（source_key）・カテゴリ別・期間別・キーワード・**イベント日（カレンダー連動）**・**エリア（地名）別**
- ソート: 新着順（published_at 降順）固定
- カード表示: タイトル（リンク）・サイト名バッジ・**開催日（`event_dates` 優先）/取得日チップ**・概要テキスト・**イベント日バッジ**・**シェア/カレンダー追加アクション**（Googleカレンダー・X・LINE）
- **カレンダー**: `event_dates` を月グリッドに集計表示し、イベントのある日をハイライト。クリックでその日の記事に絞り込み（月送りナビ付き）
- **これからのイベント**: 今日以降の開催日を持つ記事を開催日が近い順に横スクロール表示
- **エリアタグ**: タイトル・概要から区内の地名（高円寺・荻窪・西荻窪・阿佐ヶ谷ほか）をフロントで抽出し、2件以上のエリアをチップで絞り込み（西荻窪は荻窪と分離、東/新高円寺は高円寺に統合）
- **ダークモード**: トグル切替（`localStorage` 保存）＋ OS設定（`prefers-color-scheme`）に追従。初回描画前にテーマ適用してFOUC回避
- レスポンシブ対応（モバイル閲覧も想定）
